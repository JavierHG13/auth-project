<template>
  <div class="verify-container">
    ...
    <p class="instruction">Ingresa el código de 6 dígitos que enviamos a tu correo.</p>
...
const verifyCode = async () => {
  ...
  try {
    ...
    message.value = error.response?.data?.message || 'Error al verificar código'
    messageType.value = 'error'
    code.value = ''
  } finally {
    loading.value = false
...
const resendCode = async () => {
  ...
  try {
    const response = await axios.post(`${API_URL}/resend-recovery-code`, { email: email.value })
...
<style scoped>
/* Mismos estilos que VerifyEmailView.vue */
.verify-container {
  max-width: 400px;
  margin: 4rem auto;
  padding: 2rem;
